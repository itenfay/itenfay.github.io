...
![](https://itenfay.github.io/images/singleton/singleton_pattern.jpg)
...
[DYFStore](https://link.jianshu.com?t=https://github.com/itenfay/DYFStore) (In-app Purchase in Swift for iOS) 属性 default 持有唯一的实例，对外公开。
...
静态变量 _instance 持有唯一的实例，通过 defaultStore 方法对外公开。由 dispatch_once 保证 _instance 只初始化一次。方法返回值的 nonnull 表示返回值不为空，这样写方便 Swift 调用。不加 nonnull，defaultStore 方法在 Swift 中的返回值是 optional 类型 (DYFStore?)，不方便使用；若加上 nonnull，则为 [DYFStore](https://github.com/itenfay/DYFStoreKit) (In-app Purchase in Objective-C for iOS) 类型。
...
Demo ( Objective-C )：[戳这里](https://github.com/itenfay/DYFStoreKit) <br >
Demo ( Swift )：[戳这里](https://github.com/itenfay/DYFStore)
